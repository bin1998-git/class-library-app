...
// 뷰 계층에서는 DAO 를 직접 호출하지 말고 항상 Service 를 통해서 접근 한다.
public class LibraryService {
    ...
    public List<Book> searchBooksByTitle(String title) throws SQLException {
        if (title == null || title.trim().isEmpty()) {
            throw new SQLException("책제목을 입력해 주세요");
        }
    ...
    // 전체 학생 목록 조회
    public List<Student> getAllByStudent() throws SQLException {
    ...
    public int borrowBook(int bookId, int studentId) throws SQLException {
       ...
       int rows = borrowDAO.borrowBook(bookId, studentId);
        return rows;
    }
    ...
    // 현재 대출중인 도서 조회 기능
    public List<Borrow> searchBroowBookList() throws SQLException {
